...
Imagem* copiaImagem(Imagem *origem){
    ...
    */ 
    if (origem != NULL) {
        Imagem *copia = alocaImagem(origem->largura, origem->altura);   
        for (int i = 0; i < copia->altura; i++) {
            for (int j = 0; j < copia->largura; j++) {
                Pixel pixel = obtemPixel(origem, i, j);
                recolorePixel(copia, i, j, pixel);
            }
        }   
        return copia;
    }
...
void salvaImagem(Imagem *img, const char *nomeArquivo){
    ...
    fprintf(arquivo, "#EP1 - Estrutura de Dados 1 - FELIPE CAMILO SOARES\n");
